...
import com.kyawhtut.lib.rv.BindMap

class FastAdapter<T>(
    private var items: MutableList<T>,
    private var list: RecyclerView? = null,
    private var vpList: ViewPager2? = null
) : RecyclerView.Adapter<FastViewHolder<T>>() {
     ...
     */
    fun layoutManager(manager: RecyclerView.LayoutManager): FastAdapter<T> {
        vpList?.let { throw UnsupportedOperationException("layoumanager not needed for ViewPager2") }
        list!!.layoutManager = manager
        return this
    }

    fun itemChange(callback: (Int) -> Unit): FastAdapter<T> {
        this.callback = callback.also {
            it(itemCount)
        }
        return this
    }
